...
window.MikuCore = (function(){
  ...
  function createErrorDisplay(container, message = "Something went wrong!", retryFn = null){
    if (!container) return;
  ...
  return {
    $, byId, mikuIcon,
    settings: {
      isReducedMotion, isVfxEnabled, getSwallowerRate, isTypingAidsEnabled,
      get: getStorageItem, set: setStorageItem, KEYS: SETTING_KEYS
    },
    error: { createDisplay: createErrorDisplay },
    network: { fetchWithTimeout },
    telemetry: { log: logMikuEvent },
    timers: { setInterval: setTrackedInterval, setTimeout: setTrackedTimeout, clearAll: clearAllTimers }
  };
})();
