/* Cursor effects module: animated cursors and floating heart trail */
(function(){
    ...
    try {
      ...
      const boot = async () => {
        ...
        window.setBusyCursor = (on) => {
          ...
          if(on){
            if(!busyStyle){ busyStyle = document.createElement("style"); busyStyle.id = id; document.head.appendChild(busyStyle); }
            busyStyle.textContent = `html, body, * { cursor: auto !important; }`;
            applyAni("html, body, *", roleToFile.busy);
          } else if(busyStyle){ busyStyle.remove(); }
        };
        ...
        window.setWorkingCursor = (on) => {
          ...
          if(on){
            if(!w){ w = document.createElement("style"); w.id = id; document.head.appendChild(w); }
        ...
        window.setGameCursor = (mode) => {
          ...
          if(mode && roleToFile[mode]){
            if(!gameStyle){ gameStyle = document.createElement("style"); gameStyle.id = id; document.head.appendChild(gameStyle); }
