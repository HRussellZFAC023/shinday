/* Cursor effects module: floating heart trail + gated ani-cursor (preserves element animations) */
(function () {
  ...
  async function initAniCursors() {
    ...
    const boot = async () => {
      ...
      // Reuse or create the style bucket, and clear any previous (un-gated) rules
      let style = document.getElementById("ani-cursor-styles");
      if (!style) {
      ...
      // Helper: gate applied CSS so it never hits elements that must keep their own animations
      function gated(sel) {
        // :where() keeps specificity low; :not() opt-outs protect animated elements
        return `:where(${sel}):not(.preserve-anim):not([data-preserve-anim])`;
      }
      ...
      async function getAniCss(selector, fileName) {
        ...
        // Gate the selector here so the injected CSS doesn't clobber element animations
        const css = convert(gated(selector), new Uint8Array(buf));
