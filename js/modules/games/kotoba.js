...
(function(){
  ...
  // DOM
  let chatEl, cEl, fbEl, scoreEl, timerWrap, timerEl, streakEl, bestStreakEl, bestTimeEl, startBtn;
  // State
  let lock=false, tId=null, startAt=0, countdown=20; let score=0, streak=0, bestStreak=0, bestTime=null;
  const recent = []; const RECENT_LIMIT=12; function pushRecent(x){ recent.push(x); while(recent.length>RECENT_LIMIT) recent.shift(); }
  ...
  async function getQuestion(){
    ...
    if (Math.random()<0.7){
      for (let guard=0; guard<20; guard++){
        const it = deck[rnd(deck.length)]; const key = it.jp+it.en; if (recent.includes(key)) continue; pushRecent(key);
        const correct = it.en; const pool = deck.filter(x=>x!==it).map(x=>x.en);
        const decoys = shuffle(pool).slice(0, decoyCount);
        if (decoys.length<decoyCount) continue;
        return { promptHtml:`<div style=\"font-size:26px;font-weight:900\">${it.jp}</div>`, correct, options: shuffle([correct, ...decoys]) };
      }
    } else {
      try{
        const q = ['miku','song','love','friend','summer','happy'][rnd(6)];
        const data = await fetchJson(`https://jisho.org/api/v1/search/words?keyword=${encodeURIComponent(q)}`);
        const items = (data && data.data)||[];
        for (let i=0;i<items.length;i++){
          const jp = items[i].japanese?.[0]?.word || items[i].japanese?.[0]?.reading; const senses = items[i].senses?.[0]?.english_definitions||[]; if (!jp || senses.length<2) continue; const correct=senses[0];
          const key=jp+correct; if (recent.includes(key)) continue; pushRecent(key);
          const decoys = shuffle(senses.slice(1)).slice(0, decoyCount);
          if (decoys.length<decoyCount) continue;
          return { promptHtml:`<div style=\"font-size:26px;font-weight:900\">${jp}</div>`, correct, options: shuffle([correct, ...decoys]) };
        }
      }catch(_){ /* fall back */ }
    }
    // fallback single option prevents dead-end
    return { promptHtml:'<div>ことば</div>', correct:'Word', options:['Word','Phrase','Name','Sound'] };
  }

  function mount(){
    chatEl=document.getElementById('kotobaChat'); cEl=document.getElementById('kotobaChoices'); fbEl=document.getElementById('kotobaFeedback'); scoreEl=document.getElementById('kotobaScore'); timerWrap=document.getElementById('kotobaTimerWrap'); timerEl=document.getElementById('kotobaTimer'); streakEl=document.getElementById('kotobaStreak'); bestStreakEl=document.getElementById('kotobaBestStreak'); bestTimeEl=document.getElementById('kotobaBestTime'); startBtn=document.getElementById('kotobaStart');
    if (!cEl) return; try{ attachDivaHud && attachDivaHud('kotobaCard'); }catch(_){ }
    try{ bestStreak = parseInt(localStorage.getItem('kotoba.bestStreak')||'0',10)||0; }catch(_){ }
    try{ const bt=parseInt(localStorage.getItem('kotoba.bestTime')||'0',10)||0; bestTime = bt||null; }catch(_){ }
    ...
    if (startBtn) startBtn.addEventListener('click', ()=> start({ timed: isTimed() }));
  }
  ...
  function say(text, from='miku'){
    ...
    if (from==='miku'){ b.style.background='linear-gradient(45deg,#BDE3FF,#CFF6E6)'; b.style.alignSelf='flex-start'; b.textContent = 'ミク: '+text; }
    else { b.style.background='linear-gradient(45deg,#FFD1EC,#E6D1FF)'; b.style.alignSelf='flex-end'; b.textContent = 'あなた: '+text; }
  ...
  document.addEventListener('kotoba-start', (ev)=>{ const d = ev.detail || {}; start({ timed: !!d.timed }); });
  window.Games = window.Games || {}; window.Games.kotoba = { isTimed, setTimed, mount, start, stop };
})();
