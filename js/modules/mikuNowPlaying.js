(function(){
  ...
  async function loadMikus(){
    try{
      const res = await fetch('./assets/pixel-miku/mikus.json', { cache:'no-store' });
  ...
  function selectedMikuUrl(){
    try{ return localStorage.getItem('singer.current') || ''; }catch(_){ return ''; }
  }

  function normalize(p){ return (p||'').toLowerCase().replace(/\\/g,'/'); }

  function findByImage(list, url){
    const u = normalize(url);
    // match by filename containment
    return list.find(m=> u.includes(normalize(m.filename||'')) || u.includes(normalize(m.image||'')) ) || null;
  }
  ...
  function ensureDivaInfo(){
    ...
    if (hud){
      ...
      host.innerHTML = '<strong>Miku:</strong> <span id="divaInfoMiku">—</span> <div class="spacer"></div> <strong>🎶 Song:</strong> <span id="divaInfoSong">—</span>';
      hud.insertAdjacentElement('afterbegin', host);
      return host;
    }
    // Fallback: inject at top of Study/Games/Home section if HUD not yet in DOM
    const study = document.getElementById('study') || document.getElementById('games') || document.getElementById('home');
  host = document.createElement('div');
  host.id = 'divaInfo';
  host.style.cssText = 'margin:10px 0;padding:8px 10px;border:2px solid var(--border);border-radius:10px;background:#fff;display:flex;gap:12px;align-items:center;font-weight:800;color:#2b2b44';
  host.innerHTML = '<div><strong>Miku:</strong> <span id="divaInfoMiku">—</span></div><div><strong>🎶 Song:</strong> <span id="divaInfoSong">—</span></div>';
  ...
  async function refresh(){
    const url = selectedMikuUrl(); if (!url){ setInfo(null); return; }
