...
(function(){
  ...
  const QUESTS = [
    { id:'play-song',     text:'Play any song in the Jukebox', amount:1, reward:{xp:20} },
    { id:'cool-judges',   text:'Hit 10 COOL judgments',        amount:10, reward:{xp:30} },
    { id:'answers-right', text:'Answer 15 study prompts',      amount:15, reward:{xp:50} }
  ];

  function todayKey(){ return new Date().toDateString(); }
  function load(){
    try{ const raw = JSON.parse(localStorage.getItem(LS_Q)||'{}'); return raw; }catch(_){ return {}; }
  }
  function save(state){ try{ localStorage.setItem(LS_Q, JSON.stringify(state)); }catch(_){}}
  ...
  function inc(id, delta){
    ...
    it.progress = Math.min(QUESTS.find(q=>q.id===id).amount, (it.progress||0) + (delta||1));
    if (it.progress >= QUESTS.find(q=>q.id===id).amount){
      ...
      try{ if (window.SFX) SFX.play('ui.select'); }catch(_){ }
    }
    save(s);
    render();
  }
  function get(){ return ensureState(); }
  ...
  function render(){
    ...
    if (!widget){
      ...
      widget.innerHTML = '<h3>🗒️ Daily Quests</h3><div class="quests-list"></div>';
      sidebar.appendChild(widget);
    }
    const list = widget.querySelector('.quests-list');
    const s = ensureState();
    list.innerHTML = QUESTS.map(q=>{
      const it = s.items.find(i=>i.id===q.id) || {progress:0,done:false};
  ...
  function wireHooks(){
    ...
    try{
      ...
      if (el){
        el.addEventListener('click', (e)=>{
          ...
          if (b && /option|answer|choice/i.test(b.className)) inc('answers-right', 1);
        });
      }
    }catch(_){ }
    // Hook jukebox play
    try{
      ...
      if (typeof origPlay === 'function'){
        Jukebox.play = function(){ inc('play-song', 1); return origPlay.apply(this, arguments); };
      }
    }catch(_){ }
  }

  function init(){ ensureState(); render(); wireHooks(); }
