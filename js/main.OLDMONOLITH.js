...
function friendlyError(container, retryFn, opts = {}) {
  ...
  window.ShimejiFunctions?.makeRandomSpeak?.(["awww", "oops", "reload?"], 1400);
}
...
function __pumpClassify() {
  while (
    __classifyInflight < __CLASSIFY_CONCURRENCY &&
    __classifyQueue.length
  ) {
    ...
    analyzeImageUrl(url)
      .then(
        (ratios) => {
          ...
          const base = MIKU_META[url] || {};
          const hint = ratios
            ? { ...decideType(ratios), ratios }
            : { type: "Hatsune Miku", description: "" };
          const merged = {
            ...hint,
...
// ====== OPTIMIZED Wish SYSTEM ======
function initMikuWish() {
  ...
  const LS_TOKENS = "Wish.tokens";
  const LS_DAILY = "Wish.lastDaily";
  ...
  function updateDailyBtn() {
    ...
    if (last === today) {
      ...
      btn.disabled = false;
      btn.textContent = C.games?.WishDailyLabel || "Daily Ticket";
    }
  }
  ...
  function pickRandom() {
    ...
    const pool =
      buckets[pickedR] && buckets[pickedR].length
        ? buckets[pickedR]
        : MIKU_IMAGES;
      ...
      .map((c, i) => {
        return `
          <div class="Wish-card slot-machine rarity-${
            ...
            <div class="slot-reel">
              <img class="reel-image" src="${
                previewImg || c.url
              }" alt="Miku" loading="lazy" />
            </div>
  ...
  function renderDex() {
    ...
    const applySearch = (url) => {
      ...
      if (meta) {
        ...
        if (
          Array.isArray(meta.alt_names) &&
          meta.alt_names.some((n) => (n || "").toLowerCase().includes(q))
        )
          return true;
      }
    ...
    const tiles = pool
      .map((url) => {
        ...
        const vid =
          (meta &&
            meta.song &&
            (meta.song.match(/(?:v=|be\/)\b([a-zA-Z0-9_-]{11})/) || [])[1]) ||
          "";
        ...
        return `
          <div class=\"dex-card ${ownClass}\" data-url=\"${url}\" tabindex=\"0\">
            ...
            <div class=\"dex-details\" hidden>
              ...
              ${
                meta?.description
                  ? `<div class=\"dex-desc\">${meta.description}</div>`
                  : ""
              }
              ${
                vid
                  ? `<div class=\"dex-video\"><iframe src=\"https://www.youtube.com/embed/${vid}\" allow=\"accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen loading=\"lazy\" referrerpolicy=\"strict-origin-when-cross-origin\"></iframe></div>`
                  : ""
              }
    ...
    const save = () => localStorage.setItem(LS_FILTER, JSON.stringify(filters));
    if (scopeSel)
  ...
  elements.convert.addEventListener("click", () => {
    ...
    setTimeout(() => {
      if (
        ...
        if (hasPixieBel() && localStorage.getItem("pixiebelUnlocked") !== "1")
          awardPixieBel();
      }
    }, 400);
...
function initJpGames() {
  ...
  (function ensureSettingsOverlay() {
    ...
    if (!ov) {
      ...
      ov.querySelector("#startGameBtn").addEventListener("click", async () => {
        ...
        if (game === "vocab") {
          const dir =
            Array.from(vdirEls).find((r) => r.checked)?.value || "jp-en";
          ...
          const mode =
            Array.from(kmodeEls).find((r) => r.checked)?.value || "meaning";
  ...
  if (wod) {
    ...
    const configuredProxy =
      (C.api && C.api.proxyBase) || localStorage.getItem("api.proxyBase") || "";
  ...
  function ensureStageVisuals(which) {
    ...
    if (!floor) {
      ...
      floor.style.cssText =
        "position:absolute;left:0;right:0;bottom:0;height:60px;background:linear-gradient(0deg, rgba(255,255,255,0.9), rgba(255,255,255,0));pointer-events:none;z-index:1;";
  ...
  const lvl =
    ...
    parseInt(localStorage.getItem("study.level") || "1", 10) ||
    1;
  ...
  function endSong(reason) {
    ...
    const g =
      typeof HUD !== "undefined" && HUD.game
        ? HUD.game
        : localStorage.getItem("preferred.game") || "vocab";
    const mult = g === "kanji" ? 1.15 : 1.0;
    th = {
      clear: Math.round(th.clear * mult),
      ...
      const s =
        (window.Jukebox &&
          ...
          (function () {
            ...
            return (Jukebox.songs || []).find((x) => x.id === id);
          })()) ||
        null;
      if (s && s.theme) {
  ...
  function flashJudge(cardId, label) {
    ...
    if (hudJ) {
      ...
      hudJ.setAttribute("data-judge", label);
    }
    ...
    if (s) {
      const pickPhrase = () => {
        ...
        if (C.study?.wordOfDay?.japanese)
          pool.push(String(C.study.wordOfDay.japanese));
        if (Array.isArray(C.home?.heroParagraphs))
          pool.push(...C.home.heroParagraphs.map(String));
            ...
            (text, element, style) => {
              ...
              if (isCorrect) {
                ...
                const gain =
                  ...
                  mult *
                  getRhythmMult();
                const finalGain = style.isPerfect
                  ? Math.round(gain * 1.5)
                  : Math.round(gain);
                addXP(finalGain);
  ...
  (function mikuKotoba() {
    if (true) {
      ...
      async function handleChatLookup(q) {
        ...
        reply.textContent = def
          ? `Miku: ${jp?.word || jp?.reading || ""} → ${def}`
          : "Miku: (no result)";
...
function showSpotlightSweep(cardId) {
  ...
  sweep.style.cssText =
    "position:absolute;inset:0;background:linear-gradient(100deg, transparent 30%, rgba(255,255,255,0.5) 50%, transparent 70%);transform:translateX(-120%);filter:blur(2px);pointer-events:none;z-index:6";
  card.style.position = "relative";
...
function showConfetti(cardId, count = 24) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    p.style.cssText = `position:fixed;left:${
      ...
      rect.top + 10
    }px;width:6px;height:10px;background:${c};transform:rotate(${
      Math.random() * 180
    }deg);border-radius:2px;z-index:9999`;
  ...
  function lockMatched() {
    ...
    if (matchedPairs === totalPairs) {
      setTimeout(() => {
        ...
        const imgPath =
          (C && C.images && C.images.menuCovers && C.images.menuCovers.kanji) ||
          "./assets/win.jpg";
...
function activateHeartShield(ms = 3000) {
  ...
  if (!glowOverlay) {
    ...
    glowOverlay.id = "shieldGlowOverlay";
...
function showSwallowMascotGame() {
  ...
  img.src = src;
  img.alt = "swallow";
  img.className = "swallow-game-mode";
  img.style.cssText =
    "position:fixed; top:20%; left:-120px; width:96px; height:auto; z-index:9998; pointer-events:none; filter: hue-rotate(90deg) saturate(1.25) drop-shadow(2px 2px 4px rgba(0,0,0,.35));";
  img.onerror = () => {
    img.remove();
  };
  ...
  function step(ts) {
    ...
    if (hasEatenBait && p < 0.6) {
      ...
      const y = 60 + Math.sin(p * Math.PI * 0.5) * 20; // Lower and gentler
      img.style.transform = `translate(${Math.round(x)}px, ${Math.round(y)}px)`;
    } else if (isDeflected) {
      // Bounced away by shield
      const x = -120 + (W + 240) * p * 2; // Faster exit
      const y = -30 + Math.sin(p * Math.PI) * 15; // Higher arc
      img.style.transform = `translate(${Math.round(x)}px, ${Math.round(y)}px)`;
    } else {
      // Normal threatening approach
      const x = -120 + (W + 240) * p;
      const y = Math.sin(p * Math.PI) * 30;
      img.style.transform = `translate(${Math.round(x)}px, ${Math.round(y)}px)`;
      ...
      if (idol && p > 0.3 && p < 0.8) {
        ...
        if (
          kr.left < sr.right + margin &&
          kr.right > sr.left - margin &&
          kr.top < sr.bottom + margin &&
          kr.bottom > sr.top - margin
        ) {
...
function showSwallowMascotAmbient() {
  ...
  img.src = src;
  img.alt = "swallow";
  img.className = "swallow-ambient-mode";
  img.style.cssText =
    "position:fixed; bottom:10px; left:-120px; width:96px; height:auto; z-index:9997; pointer-events:none; transform: scaleX(-1); filter: hue-rotate(45deg) saturate(1.1) drop-shadow(2px 2px 4px rgba(0,0,0,.25));";
  img.onerror = () => {
    img.remove();
  };
  document.body.appendChild(img);
  ...
  function step(ts) {
    ...
    shimejis.forEach((shimeji) => {
      ...
      if (
        ...
        // Shimeji reaction
        shimeji.style.animation = "bounce 0.6s ease-out";
        setTimeout(() => {
...
function showSwallowMascot() {
  // Decide which version to show based on context
  if (
    (window.location?.hash || "").includes("study") ||
    document.getElementById("jpGames")
  ) {
...
// Automatic swallower spawning system
let __swallowerGameTimer = null;
let __swallowerAmbientTimer = null;

function startSwallowerTimers() {
...
window.addEventListener("load", startSwallowerTimers);
document.addEventListener("DOMContentLoaded", startSwallowerTimers);
...
window.addEventListener("load", enhanceGlobalInteractions);
document.addEventListener("DOMContentLoaded", enhanceGlobalInteractions);

// Random celebration system
...
function initShrine() {
  ...
  function renderGallery() {
    ...
    mikuGallery.innerHTML = galleryItems
      .map((img, index) => {
        ...
        const coverText =
          isPixieSlot && !pixieOwned ? '<div class="mystery-text">?</div>' : "";
  ...
  if (songList) {
    ...
    songList.querySelectorAll("li").forEach((li) => {
      if (!li.classList.contains("favorite-song"))
        li.classList.add("favorite-song");
      if (!li.getAttribute("data-title"))
        li.setAttribute("data-title", li.textContent.trim());
    });
  }
}
...
function applySingerTheme() {
  ...
  const r = url
    ...
    : 0;
  const color =
    r >= 5 ? "#ffb300" : r >= 4 ? "#a594f9" : r >= 3 ? "#6bc3ff" : "#8fd3a8";
  const prog = document.getElementById("hudLevelProgress");
  if (prog) prog.style.background = `linear-gradient(45deg, ${color}, #ffffff)`;
  // HUD singer image removed
}
