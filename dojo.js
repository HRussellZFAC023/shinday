...
(function () {
  ...
  class QuestionGenerator {
    // Return a cumulative pool for a given JLPT kind (e.g., 'vocab' or 'kanji')
    // by concatenating packs from level 1 up to the current difficulty.
    static pool(kind, upto = State.difficulty) {
      const src = (window.JLPT || {})[kind] || {};
      const max = Math.max(1, upto | 0);
      let out = [];
      for (let i = 1; i <= max; i++) {
        const pack = src[i];
        if (Array.isArray(pack) && pack.length) out.push(...pack);
      }
      return out;
    }
    static async vocab() {
      await (window.JLPT_READY || Promise.resolve());
      const pool = QuestionGenerator.pool("vocab");
    ...
    static async kanji() {
      await (window.JLPT_READY || Promise.resolve());
      const pool = QuestionGenerator.pool("kanji");
      if (!pool.length) return null;
      const entry = pool[Math.floor(Math.random() * pool.length)];
  ...
  class HUDManager {
    ...
    flashJudge(cardId, label) {
      ...
      if (label === "COOL") window.Quests?.inc?.("cool-judges", 1);
